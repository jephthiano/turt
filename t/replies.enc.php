...
$page_url = file_location('home_url',@$_GET['type'].'/replies/'.@$_GET['cid']);
...
if(isset($_GET['type']) && isset($_GET['cid']) && is_numeric($_GET['cid'])){//getting the value of the get
	$cid = ($_GET['cid']);
	$ttype = ($_GET['type']);
	if(!empty($cid) && !empty($ttype)){
		$comment_id = $cid;
		$type = $ttype;

<?php
require_once($_SERVER['DOCUMENT_ROOT'].'/addons/function.inc.php');// all functions
require_once(file_location('inc_path','session_check_nologout.inc.php'));
$missing = []; $data = [];
if(isset($_POST) && isset($_POST['id'])){
	//validating mobile number or username
	$id = strtolower($_POST['id']);
	if(empty($id)){
		$missing['ide'] = "* Id cannot be empty";
	}else{
		$val = get_id_column($id);
		if($val === false){
			$missing['ide'] = 'invalid_id';
		}else{
			$col = $val;
			$en_id = get_id_true_data($id,$col);//encyrpt if id is phnumber or email
		}
		if(empty($missing)){ // check if does not exists
			if(content_data('user_table','u_id',$en_id,$col) === false){
				if($col === "u_phnumber"){
					$missing['ide'] = "* Mobile number not exists";
				}elseif($col === "u_email"){
					$missing['ide'] = "* Email not exists";
				}else{
					$missing['ide'] = "* Username not exists";
				}
			}else{
				$login_id = (test_input($id));
			}
		}
	}
	
	if(empty($missing)){
		//SET ID COOKIE AND REDIRECT TO THE SECOND PAGE
		set_single_cookie("forgot_password_id",$login_id);
		$data["status"] = 'success';$data["message"] = file_location('home_url','forgot_password/medium/');
	}else{
		$data["status"] = 'error';$data["errors"] = $missing;
	}
}else{
	$data["status"] = 'fail';$data["message"] = 'Error occurred while processing id';
}//end of if empty
echo json_encode($data);
?>